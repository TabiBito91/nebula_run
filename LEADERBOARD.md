...
Ranking: score descending, victories before defeats on ties, oldest server timestamp then run ID. Both ship variants participate because existing scoring is unchanged. `BOARD_VERSION` in `src/game/leaderboard/rules.ts` isolates balance/route changes; bump it when those rules change. Older online rows remain stored but are not displayed on the new board; local lists retain only the current version.
