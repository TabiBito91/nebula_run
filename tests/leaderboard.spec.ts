...
test('fixtures and abandoned sorties never record or submit',async({page})=>{
  ...
  await expect(page.getByRole('button',{name:'Share score online'})).toHaveCount(0)
...
test('real sortie records once, retries an ambiguous submission safely, and persists locally',async({page},info)=>{
  ...
  await page.route('**/api/**',async r=>{
    const path=new URL(r.request().url()).pathname
    const response=await r.fetch({url:`http://127.0.0.1:${server.address().port}${path}`})
  ...
  await expect(page.getByRole('button',{name:'Share score online'})).toBeVisible()
  expect((await state(page)).leaderboard.localCount).toBe(1);expect(submitted).toBe(0)
  const name=page.getByRole('textbox',{name:'Public display name (optional)'})
  await name.fill('Nova Pilot_7')
  await page.getByRole('button',{name:'Share score online'}).click()
  await expect(page.getByRole('status')).toContainText('retry safely')
  await page.getByRole('button',{name:'Share score online'}).click()
  await expect(page.getByRole('button',{name:'Score shared'})).toBeDisabled()
