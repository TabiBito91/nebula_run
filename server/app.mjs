...
import { BOARD_VERSION, BOARDS, boardVersion, currentVersion, scoreMultiplier, ONLINE_LIMIT, validResult } from '../src/game/leaderboard/rules.ts'
...
export function createApp({ store, origin, secret, trustProxyHops = 0, now = Date.now, staticDir = 'dist' }) {
  ...
  async function body(req, limit = 4096) {
    ...
    for await (const chunk of req) { text += chunk; if (Buffer.byteLength(text) > limit) throw bad(413, 'Request too large') }
  ...
  return createServer(async (req, res) => {
    ...
    try {
      ...
      if (!['/api/leaderboard','/api/runs','/api/scores','/api/feedback'].includes(path)) throw bad(404, 'Not found')
      if (path === '/api/feedback' && req.method !== 'POST') throw bad(405, 'Method not allowed')
      ...
      if (!await store.allow(key, req.method === 'GET' ? 120 : path === '/api/feedback' ? 5 : path === '/api/runs' ? 12 : 40, (bucket + 2) * period)) throw bad(429, 'Too many requests; try later')
      if (path === '/api/feedback') {
        const data = validateFeedback(await body(req, 8192))
        if (!data) throw bad(400, 'Invalid feedback')
        if (!await store.feedback(data, hash(JSON.stringify(data)), time)) throw bad(409, 'Submission ID already used')
        return json(res, 200, { accepted:true, id:data.id })
      }
      if (path === '/api/leaderboard' && req.method === 'GET') {
        const version = new URL(req.url, 'http://localhost').searchParams.get('version') || BOARD_VERSION
        if (!BOARDS.some(b => b.version === version)) throw bad(400, 'Unsupported board')
        return json(res, 200, { version, entries:await store.top(version, ONLINE_LIMIT) })
      }
      if (path === '/api/runs' && req.method === 'POST') {
        const b = await body(req)
        if (!currentVersion(b.version) || Object.keys(b).some(k=>k!=='version')) throw bad(400, 'Unsupported run version')
        await store.prune(time)
        const id = randomUUID(), token = randomBytes(32).toString('hex'), expires = time + 7200000
        await store.create({ id, hash:hash(token), version:b.version, started:time, expires })
        return json(res, 201, { id, token, expires })
      }
      if (path === '/api/scores' && req.method === 'POST') {
        const b = await body(req)
        if (!validResult(b) || !currentVersion(b.version) || typeof b.id !== 'string' || !/^[a-f0-9-]{36}$/.test(b.id)
        ...
        if (!run || run.version !== b.version) throw bad(403, 'Invalid run')
        if (run.score !== null) {
        ...
        const waveBudget = b.version === boardVersion('veteran') ? 600 : 400
        const asteroidBudget = Math.max(0, Math.min(11, Math.ceil((b.elapsed - 55) / 3.2))) * 40
        const scoreLimit = (Math.ceil(b.elapsed / 4.5) * waveBudget + asteroidBudget + (b.outcome === 'victory' ? 2000 : 0)) * scoreMultiplier(b.version)
        if (b.elapsed > (time - Number(run.started)) / 1000 + 8 || b.score > scoreLimit) throw bad(400, 'Implausible score or duration')
      ...
      if (!res.headersSent) json(res, error.status || 503, { error:error.status ? error.message : req.url?.split('?')[0] === '/api/feedback' ? 'Feedback unavailable; please retry later' : 'Leaderboard unavailable; local scores remain available' })
      else res.end()
    }
  })
}
