...
export class AudioControls {
  ...
  constructor(audio: AudioManager, clearInput: () => void) {
    ...
    this.root.innerHTML = `<button type="button" aria-expanded="false" aria-controls="audio-settings">Audio</button>
      <section id="audio-settings" aria-label="Audio settings" hidden><strong>FLIGHT AUDIO</strong><p data-audio-status>Sound starts after interaction.</p>
  ...
  private refresh() {
    ...
    this.root.querySelector('[data-audio-status]')!.textContent = !state.available ? 'Audio unavailable. Flight remains playable.' : state.unlocked ? 'Audio ready. Settings saved on this device.' : 'Sound starts after interaction.'
  }
